...
void separableJointBilateralFilter(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_space, double alpha, int method, int borderType)
{
	if(method==DUAL_KERNEL_HV)
	{
		jointBilateralFilter(src, guide, dst, Size(kernelSize.width,1), sigma_color, sigma_space, FILTER_RECTANGLE, borderType);
		jointBilateralFilter(dst, guide, dst, Size(1, kernelSize.height), sigma_color*alpha, sigma_space, FILTER_RECTANGLE, borderType);
	}
	else if(method==DUAL_KERNEL_VH)
	{
		jointBilateralFilter(src, guide, dst, Size(1,kernelSize.height), sigma_color, sigma_space, FILTER_RECTANGLE, borderType);
		jointBilateralFilter(dst, guide, dst, Size(kernelSize.width,1), sigma_color*alpha, sigma_space, FILTER_RECTANGLE, borderType);
	}
	else if(method==DUAL_KERNEL_HVVH)
	{
		...
		jointBilateralFilter(dst, guide,  dst, Size(1, kernelSize.height), sigma_color*alpha, sigma_space, FILTER_RECTANGLE, borderType);
		Mat dst2;
		jointBilateralFilter(src,  guide,  dst2, Size(1, kernelSize.height), sigma_color, sigma_space, FILTER_RECTANGLE, borderType);
		jointBilateralFilter(dst2, guide, dst2, Size(kernelSize.width,1), sigma_color*alpha, sigma_space, FILTER_RECTANGLE, borderType);

		alphaBlend(dst,dst2,0.5,dst);
	}
	else if (method==DUAL_KERNEL_CROSS)
	{
		jointBilateralFilter_direction_8u(src, guide, dst, kernelSize, sigma_color, sigma_space, borderType, 1, true);
		jointBilateralFilter_direction_8u(dst, guide, dst, kernelSize, sigma_color*alpha, sigma_space, borderType, -1, true);
	}
	else if (method==DUAL_KERNEL_CROSSCROSS)
	{
		...
		jointBilateralFilter_direction_8u(src, guide, dst2, kernelSize, sigma_color, sigma_space, borderType, -1, true);
		jointBilateralFilter_direction_8u(dst2, guide,  dst2, kernelSize, sigma_color*alpha, sigma_space, borderType, 1, true);
