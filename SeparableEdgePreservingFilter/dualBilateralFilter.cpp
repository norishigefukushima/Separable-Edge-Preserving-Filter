...
void trilateralWeightMapXOR_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color, double sigma_space, int borderType )
{
	int cn = src.channels();
	int cng = guide.channels();
	int i, j, maxk;
...
void trilateralWeightMapSGB_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color, double sigma_space, int borderType )
{
	int cn = src.channels();
	int cng = guide.channels();
	int i, j, maxk;
...
void trilateralWeightMapXOR(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space,int method, int borderType)
{
	if(dst.empty())dst.create(src.size(),CV_32F);
	if(method==FILTER_CIRCLE)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralWeightMapXOR_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			//trilateralWeightMap_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
...
/*
void trilateralWeightMapSGB(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space,int method, int borderType)
{
	if(dst.empty())dst.create(src.size(),CV_32F);
	if(method==BILATERAL_NORMAL)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralWeightMapSGB_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			//trilateralWeightMap_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
...
void separableDualBilateralFilter(const Mat& src, const Mat& guide, Mat& dst, int D, double sigma_color, double sigma_guide_color, double sigma_space, double alpha1, double alpha2, int method, int borderType)
{
	separableDualBilateralFilter(src, guide, dst, Size(D,D), sigma_color, sigma_guide_color, sigma_space, alpha1, alpha2, method, borderType);
}